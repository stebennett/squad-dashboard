...
func (p *PostgresJiraRepository) GetNonWorkingDays(ctx context.Context, project string) ([]time.Time, error) {
	...
	return result, nil
>>>>>>> e01cff9 (Escaped defects counter completed)
}

func (p *PostgresJiraRepository) GetEscapedDefects(ctx context.Context, project string, issueType string, startDate time.Time, endDate time.Time) ([]string, error) {
	selectStatement := `
		SELECT jira_issues_calculations.issue_key
		FROM jira_issues_calculations
...
func (p *PostgresJiraRepository) GetThroughput(ctx context.Context, project string, issueTypes []string, startDate time.Time, endDate time.Time) ([]string, error) {
	selectStatement := `
		SELECT jira_issues_calculations.issue_key
		FROM jira_issues_calculations
		JOIN jira_issues ON jira_issues_calculations.issue_key = jira_issues.issue_key
