...
func CalculateTrendlineForWeeklyTimeItems(items []statsmodels.WeeklyTimeItem) ([]statsmodels.TrendlineItem, error) {
	series := stats.Series{}
...
func CalculateTrendlineForCycleTimes(items []statsmodels.WeeklyCycleTimeItem) ([]statsmodels.TrendlineItem, error) {
	series := stats.Series{}
	for _, item := range items {
		coord := stats.Coordinate{
			X: float64(item.WeekStarting.Unix()),
